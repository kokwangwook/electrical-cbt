...
**코드를 수정하거나 기능을 추가하기 전에 반드시 확인하세요:**

> 📖 **작업 방법**: [작업가이드.md](./작업가이드.md) 파일을 먼저 읽어주세요.  
> 🔐 **중요 문서**: 작업 후 뭔가 바뀐 게 있으면 `.readme_ecbt.md` 파일을 꼭 수정해주세요.

### 🚨 꼭 해야 할 일

1. **작업 시작 전**: [작업가이드.md](./작업가이드.md) 파일 읽기
2. **작업하면서**: 비밀번호, 주소, 데이터베이스 등이 바뀌면 적어두기
3. **작업 끝난 후**: 바뀐 게 있으면 `.readme_ecbt.md` 파일 수정하기
   - 파일 위치: 프로젝트 폴더 안의 `.readme_ecbt.md` (숨김 파일)
   - 수정 방법: [작업가이드.md](./작업가이드.md) 참고

### 📋 작업 끝나기 전에 확인

- [ ] [작업가이드.md](./작업가이드.md) 읽었음
- [ ] 코드 수정 완료
- [ ] **`.readme_ecbt.md` 파일 수정** (바뀐 게 있는 경우)
- [ ] Git에 저장 (`.readme_ecbt.md`는 자동으로 제외됨)
...
> 📖 **운영 가이드**: [OPERATION_GUIDE.md](./OPERATION_GUIDE.md) 참고

---

## 📋 최근 수정사항

### 2025-11-14 - 사용자 경험 최적화 및 UI 개선
- ✅ **로그인 속도 대폭 개선**:
  - Login.tsx에서 Google Sheets 동기화 로직 완전 제거
  - 문제 현황 확인 제거로 로딩 시간 수 초 → 밀리초로 단축
  - 버튼 텍스트 간소화: "📊 문제 현황 확인 중..." → "로그인 중..."

- ✅ **시험 문제 선택 UI 전면 개선**:
  - 카테고리별 그룹화: 전기이론(1-20), 전기기기(21-40), 전기설비(41-60)
  - 한 줄에 20개 문제 번호 표시 (grid-cols-20)
  - 원형 버튼 디자인 (aspect-square rounded-full)
  - 카테고리별 색상 구분: 보라색(전기이론), 파란색(전기기기), 초록색(전기설비)
  - 안내 문구 추가: "💡 문제번호를 클릭하시면 문제로 바로 이동할 수 있습니다"

- ✅ **인쇄 기능 최적화**:
  - 보기 항목 2열 레이아웃: 짧은 텍스트일 때 한 줄에 2개 표시
  - 페이지 여백 최적화: break-inside: avoid-page로 불필요한 페이지 넘김 방지
  - 여백 축소: 0.4cm → 0.3cm, 폰트 크기 16px → 14px, 줄간격 1.5 → 1.3
  - 정답 표시 개선: 체크마크(✓) 제거, 파란색으로 강조

- ✅ **문제 현황 실시간 업데이트**:
  - localStorage에서 직접 읽어 관리자/사용자 페이지 실시간 동기화
  - 페이지 포커스 시 자동 업데이트 (탭 전환 시)
  - 수동 새로고침 버튼 추가 (🔄 새로고침)
  - questionStats.json 파일 시스템 제거 (불필요)

- ✅ **출제기준 일괄 적용 기능**:
  - "📚 모든 문제에 출제기준 일괄 적용" 버튼 추가
  - 키워드 매칭으로 자동 출제기준 적용
  - 출제기준 없는 문제에 랜덤 적용
  - 세부항목 자동 적용

- ✅ **Google Sheets 동기화 최적화**:
  - Home.tsx에서 Google Sheets 연결 로직 완전 제거 (170줄 삭제)
  - 동기화는 관리자 페이지에서만 수동 실행
  - 사용자 로그인 시 즉시 로드 (동기화 대기 없음)

**변경된 파일:**
- `src/pages/Login.tsx` - Google Sheets 동기화 제거, 로딩 UI 간소화
- `src/pages/Home.tsx` - 동기화 로직 제거(170줄), 문제 현황 자동 업데이트, 새로고침 버튼
- `src/pages/Exam.tsx` - 문제 선택 UI 개선, 인쇄 최적화
- `src/pages/Admin.tsx` - 출제기준 일괄 적용, questionStats.json 업데이트 제거
- `src/index.css` - 인쇄 CSS 최적화

**사용자 경험 개선:**
```
로그인 → 즉시 완료 (밀리초) ✅
문제 현황 → 실시간 표시 (5012개) ✅
문제 선택 → 카테고리별 20개씩 한눈에 ✅
인쇄 → 페이지 낭비 없이 최적화 ✅
관리자 → 출제기준 일괄 적용 ✅
```
...
- **관리자**: http://localhost:5173/admin (비밀번호는 코드에서 확인)
...
### 7️⃣ 관리자 페이지
- 비밀번호: 코드에서 확인 (`src/pages/Admin.tsx`의 `ADMIN_PASSWORD`)
